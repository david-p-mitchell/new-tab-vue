...
export default {
  ...
  components: { CurrentStats, NowOverview, TodayRange, DailyForecast, HourlyForecast },
  data() {
    return {
      city: 'Chatham',
      cityInput: '',
  ...
  computed: {
    temp()      { return this.rawTempC !== null ? this.convertTemp(this.rawTempC) : '--' },
    feelsLike() { return this.rawFeelsC !== null ? this.convertTemp(this.rawFeelsC) : '--' },
    wind()      { return this.rawWindKph !== null ? Math.round(this.rawWindKph) : '--' },
    uvIndex()   { return this.rawUV !== null ? Math.round(this.rawUV) : '--' },
    visibility(){ return this.rawVisibility !== null ? (this.rawVisibility / 1000).toFixed(1) : '--' },
    pressure()  { return this.rawPressure !== null ? Math.round(this.rawPressure) : '--' },
    dewPoint()  { return this.rawDewC !== null ? this.convertTemp(this.rawDewC) : '--' },
    cloudCover(){ return this.rawCloudCover !== null ? Math.round(this.rawCloudCover) : '--' },
    sunrise()   { return this.sunriseStr },
    sunset()    { return this.sunsetStr },
    todayHi()   { return this.todayHiC !== null ? this.convertTemp(this.todayHiC) : '--' },
    todayLo()   { return this.todayLoC !== null ? this.convertTemp(this.todayLoC) : '--' },

    timeStr() {
      return this.now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })
    },
    dateStr() {
      return this.now.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })
    },
    ...
    mainIcon() { return this.codeToSvg(this.weatherCode, 22) },
    ...
    hilowDotPos() {
      if (this.rawTempC === null || this.todayHiC === null || this.todayLoC === null) return '50%'
  ...
  methods: {
    ...
    startEdit() {
      ...
      this.$nextTick(() => this.$refs.cityInputRef?.focus())
    },
    ...
    dayFillStyle(d) {
      ...
      const left = ((d.loC - allLo) / range * 100).toFixed(1) + '%'
      const right = (((allHi - d.hiC) / range) * 100).toFixed(1) + '%'
      return { left, right }
    },
    ...
    async fetchWeather() {
      ...
      try {
        const geoRes = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(target)}&countryCode=GB&count=4&language=en&format=json`)
        const geo = await geoRes.json()
        console.log('Geocoding API response:', geo)
        ...
        console.log('Weather API response:', wxRes)
        ...
        this.hourly = wx.hourly.time.slice(nowHour, nowHour + 24).map((t, idx) => {
          const i = nowHour + idx
          const tempC = wx.hourly.temperature_2m[i]
          const barPct = maxT === minT ? 50 : Math.round(((tempC - minT) / (maxT - minT)) * 70 + 15)
          const dt = new Date(t)
          return {
            time: idx === 0 ? 'Now' : dt.getHours().toString().padStart(2,'0') + ':00',
            isNow: idx === 0,
            tempC,
            icon: this.codeToSvg(wx.hourly.weather_code[i], 22),
            precip: Math.round((wx.hourly.precipitation?.[i] || 0) * 10) / 10,
            barPct,
          }
        })
        ...
        this.daily = wx.daily.time.map((t, i) => {
          const d = new Date(t)
          return {
            ...
            precip: Math.round((wx.daily.precipitation_sum?.[i] || 0) * 10) / 10,
          }
        })
    ...
    codeToSvg(code, size = 40) {
      const s = size, sw = size > 40 ? 1.4 : 1.8
